---
title: "Preparation 10 -- Activity 8"
author: "Name"
...
starwars %>% 
  ...
  ggplot(mapping = aes(x = eye_color)) +  geom_bar()
...
```{r}
(today() - mdy(bradford_birthday)) %/% dweeks(1)

(today() - mdy(bradford_birthday)) %/% ddays(7)
```
...
```{r first_days}
first_days_2021 <- as_datetime("2021-01-01") + months(0:11)
...
```{r weekdays}
first_days_wday <- lubridate::wday(first_days_2021, label = TRUE)
first_days_wday
